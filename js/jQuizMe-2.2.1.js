...
(function($){	
	...
	_lang = {
			ans:{	// _lang.ans are shown during the display of .q-result, the answer result.
				...
				retry: 'Jawaban masih salah<br/>Silahkan pilih jawaban anda',
				whyAns: "Info:",
				yourAns: "Jawaban anda:"
			},
			btn:{	// [ "text", "title" ]
				begin: [ "Begin Quiz" ],
				check: [ "Jawab", "Check your answer" ],
				del: [ "Delete", "Delete quiz" ],
				help: [ "Help", 'Click for help'],
				next: [ "Selanjutnya", "Next question" ],
				restart: [ "Ulangi", "Ulangi quiz" ],
				details: [ "Detail", "Lihat total nilai" ],
				review: [ "Review", "Review Questions" ],
				showOnlyMissed: [ " *", "Click to show only missed questions." ],
				quit: [ "Keluar", "Keluar quiz" ],
				quitNo: [ "->", "Kembali" ],
				quitYes: [ '', "Keluar" ]
			},
			err:{
	...
	$.fn.jQuizMe = function( wordList, options, userLang ){
		...
		return this.each( function(){
			...
			getUserStatDetailsForDisplay = function(){
				return [
						("Nilai"+ ": " + parseInt(stats.numOfRight)*2), 
